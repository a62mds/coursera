...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    if (p(this.elem)) 
      ...
      this.left.filterAcc(p, right.filterAcc(p, acc))
  }
  ...
  def mostRetweeted: Tweet = {
    lazy val leftMost = this.left.mostRetweeted
    lazy val rightMost = this.right.mostRetweeted

    if (!this.left.isEmpty && leftMost.retweets > this.elem.retweets)
      if (!this.right.isEmpty && rightMost.retweets > leftMost.retweets)
        rightMost
      else
        leftMost
    else if (!right.isEmpty && rightMost.retweets > this.elem.retweets)
      rightMost
    else
      this.elem
  }
