...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    def timesAcc(chars:List[Char],acc:List[(Char,Int)]):List[(Char,Int)] = chars match {
      case List() => acc
      case c :: cs => timesAcc(cs, addCount(c, acc))
    }
    def addCount(c: Char, count: List[(Char,Int)]): List[(Char,Int)] = count match {
      case List() => count
      case pair :: pairs => { 
        if (pair._1==c) (c,pair._2+1) :: pairs
        else addCount(c,pairs)
      }
    }
    timesAcc(chars, List())
  }
